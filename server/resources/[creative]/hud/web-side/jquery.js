...
$(document).ready(function(){
	window.addEventListener("message",function(event){
		...
		if (event["data"]["weapons"] !== undefined){
			if (event["data"]["weapons"] == true){
				if ($(".displayWeapon").css("display") === "none"){
					$(".displayWeapon").css("display","block");
				}

				$(".displayWeapon").html(event["data"]["name"] + "<br>" + event["data"]["minAmmo"] + " / " + event["data"]["maxAmmo"]);
			} else {
				if ($(".displayWeapon").css("display") === "block"){
					$(".displayWeapon").css("display","none");
				}
			}
		...
		if (event["data"]["vehicle"] !== undefined){
			...
			var teaText = ""
			if (event["data"]["tea"] < 3600){
				teaText = "<text>"+ minimalTimers(event["data"]["tea"]) +" Chá</text>"
			}
			...
			$("#Streets").html(event["data"]["street01"] + event["data"]["street02"]);

			$("#displayBoxes").html(lastRadio + teaText + reposeText + wantedText + userVoice);
		}

		if (event["data"]["parachute"] !== undefined){
			if (event["data"]["parachute"] == 1){
				if ($("#parachute").css("display") === "none"){
					$("#parachute").css("display","block");
				}
			} else {
				if ($("#parachute").css("display") === "block"){
