...
local nameContainer = "9999"
local createdContainer = false
local containerTimer = 20 * 60
-----------------------------------------------------------------------------------------------------------------------------------------
-- CONTAINERCOORDS
-----------------------------------------------------------------------------------------------------------------------------------------
local containerCoords = {
	{ 1392.33,2567.49,38.35,314.65 },
	{ -1547.94,1389.46,125.41,133.23 },
	{ -2597.11,3603.12,4.26,325.99 },
	{ 64.74,5006.91,460.72,269.3 },
	{ 3158.56,5121.41,19.04,127.56 },
	{ 405.3,6601.59,28.21,0.0 },
	{ 55.61,3713.44,39.75,289.14 },
	{ 1401.93,-2058.54,52.0,297.64 },
	{ -1523.9,-557.6,33.28,110.56 },
	{ 2674.83,1397.49,24.57,351.5 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- CONTAINERTHREAD
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
	while true do
		if not createdContainer then
			local index = math.random(#containerCoords)
			local x,y,z,h = containerCoords[index][1],containerCoords[index][2],containerCoords[index][3],containerCoords[index][4]

			Objects[nameContainer] = { x = mathLegth(x), y = mathLegth(y), z = mathLegth(z)-0.97, h = mathLegth(h), object = "prop_container_03b", distance = 150, mode = "Containers" }

			TriggerClientEvent("objects:Adicionar",-1,nameContainer,Objects[nameContainer])

			TriggerClientEvent("Notify",-1,"amarelo","<b>Container</b> disponível.",3000)
			createdContainer = true
		else
			containerCount = containerCount + 1

			if containerCount > containerTimer then
				containerCount = 0
				createdContainer = false

				TriggerClientEvent("objects:Remover",-1,nameContainer)
			end
		end
